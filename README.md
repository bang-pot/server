...
`GET /api/users/me/withdrawal-check` is the login-required pre-withdrawal validation endpoint for the account settings flow. It returns `canWithdraw`, `blockingActiveCrews`, and `blockingParticipatingMeetings`. `canWithdraw` is `true` only when the current user has no blocking active crews. `blockingActiveCrews` contains every current `ACTIVE` crew membership on `ACTIVE` crews, regardless of whether the user is a leader or a member. In the current implementation, `blockingParticipatingMeetings` is returned as an empty list for response-shape compatibility and is not used as a withdrawal blocker. This endpoint is read-only and only explains whether withdrawal is currently blocked by active crew memberships.
