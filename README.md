...
- `POST /api/meetings/{meetingId}/logs/photos`
...
`PATCH /api/users/me` updates the current user's nickname and then reloads the same profile hub payload as `GET /api/users/me`, so the response shape stays identical to the main profile read endpoint instead of returning partial `null` fields.
`GET /api/users/me/favorites/summary` is the login-required my-page hub summary endpoint for the "favorited themes" section. It returns `{items, totalCount, hasMore}`. `items` are limited to the latest five current favorites, ordered by `theme_favorites.created_at desc`, then `theme_id desc`, so the frontend can render a small recency-based summary plus a stable "view all" affordance. Each item includes `themeId`, `themeName`, `storeName`, `regionName`, nullable `thumbnailUrl`, `favoriteCount`, and `isFavorite`. `thumbnailUrl` reuses the same theme poster image as explore cards and home preview, `favoriteCount` reuses `themes.favorite_count`, and `isFavorite` is kept in the contract for theme-card consistency even though every summary item is always currently favorited. Inactive themes are excluded using the same active-theme rule as explore reads. `totalCount` is the total number of current active favorites, and `hasMore` is `true` only when that total exceeds five.
`GET /api/users/me/favorites` is the login-required profile detail endpoint for the full "favorited themes" list behind the my-page summary's "view all" entry. It returns `{items, pageInfo}` with `page`, `size`, and `hasNext`. The list includes only the current user's current favorites that still point to active themes, so unfavorited rows and inactive themes are excluded using the same source of truth as the summary and explore reads. Sorting is fixed to latest favorite first using `theme_favorites.created_at desc`, then `theme_id desc`, which matches the recency rule used by the summary endpoint. Each item includes `themeId`, `themeName`, `storeName`, `regionName`, nullable `thumbnailUrl`, `favoriteCount`, and `isFavorite`. `thumbnailUrl` reuses `themes.poster_image_url`, `favoriteCount` reuses `themes.favorite_count`, and `isFavorite` stays in the contract for card-component consistency even though every row in this list is currently favorited. The default page size is `20`, the maximum size is `50`, and the frontend should use `hasNext` for incremental loading.
...
`GET /api/users/me/withdrawal-check` is the login-required pre-withdrawal validation endpoint for the account settings flow. It returns `canWithdraw` and `blockingActiveCrews`. `canWithdraw` is `true` only when the current user has no blocking active crews. `blockingActiveCrews` contains every current `ACTIVE` crew membership on `ACTIVE` crews, regardless of whether the user is a leader or a member. This endpoint is read-only and only explains whether withdrawal is currently blocked by active crew memberships.
`POST /api/users/me/withdrawal` is the login-required withdrawal execution endpoint. It accepts `reasonCode`, optional `reasonDetail`, and `confirmationChecked=true`. Supported reason codes are `NOT_USING`, `SERVICE_UNSATISFIED`, `LOW_ACTIVITY`, and `OTHER`. The server reruns the same active-crew blocking policy as `GET /api/users/me/withdrawal-check` immediately before execution and returns `409 AUTH_WITHDRAWAL_NOT_ALLOWED` when withdrawal is still blocked by active crew memberships. On success, the backend stores a withdrawal history row, soft-deletes the current `users` row by de-identifying profile data and setting `withdrawn_at`, hard-deletes the current `auth_users` row, returns `{withdrawnAt, canLogin:false}`, and also sends an expired auth cookie so the current session is logged out immediately. Record-style data linked by `userId` remains, and a later rejoin starts as a fresh login state rather than restoring the old account profile.
`GET /api/explore/themes`, `GET /api/explore/themes/{themeId}`, and `GET /api/explore/filters` are public read endpoints and do not require authentication. Explore search uses one keyword `q` across theme name, store name, region, and district; supports repeated `genres` params plus optional `region`, `district`, `page`, and `size`; and returns `{items, pageInfo}` with `hasNext` so the frontend can extend it to infinite scroll. Explore card-like theme responses are now aligned around the same current-user favorite contract: explore list cards expose `isFavorite`, theme detail exposes top-level `isFavorite`, and `relatedThemes` also include `favoriteCount` plus `isFavorite`. When the request is unauthenticated, every card-like theme response returns `isFavorite=false`; when the request is authenticated, favorite state is resolved from the current user's theme-favorite relation without an extra API call. Theme detail still returns the core theme fields plus `relatedThemes`, where recommendations are limited to at most four other active themes from the same store, excluding the current theme itself. Missing `posterImageUrl`, `description`, and `externalLink` are returned as `null` so the frontend can handle fallback copy and imagery. `POST /api/themes/{themeId}/favorite` and `DELETE /api/themes/{themeId}/favorite` are login-required favorite mutation endpoints. Both are idempotent: repeated `POST` on an already-favorited theme succeeds as a no-op, and repeated `DELETE` on an already-unfavorited theme also succeeds as a no-op. Unfavorite uses hard delete on the `theme_favorites` relation table, and both responses return `{themeId, isFavorite, favoriteCount}` so the frontend can update card/detail state immediately.
...
`POST /api/meetings/{meetingId}/logs/photos` is a login-required meeting-log helper endpoint for image upload. The `meeting` domain keeps the upload policy, but actual file persistence is delegated to S3 through the common storage layer. It accepts a single multipart `file`, validates `jpg`/`jpeg`/`png` only, enforces `<= 5MB`, stores the object under `log-photos`, and returns `{url, sizeBytes}` so the frontend can reuse the existing `photos` array contract without change.
...
`GET /api/crews/{crewId}/schedule` is the joined-member-only crew schedule read endpoint for the crew page calendar/detail flow. It requires `from` and `to` query params in `yyyy-MM-dd` format and returns `{items}` for all meetings on that crew within the requested date range. Included meeting statuses are `RECRUITING`, `RECRUITMENT_CLOSED`, `COMPLETED`, and `CANCELED`, so the frontend can render planned, completed, and canceled schedules together. Items are ordered by `date asc`, `time asc`, then `meetingId asc`, and each item includes `meetingId`, `themeName`, `date`, `time`, `meetingStatus`, `recruitmentStatus`, `place`, `participantCount`, and `isCanceled`. `isCanceled` is `true` only when `meetingStatus = CANCELED`. `recruitmentStatus` is `OPEN` only for `RECRUITING`; every other included status is exposed as `CLOSED`. `participantCount` follows the same joined-count rule used by automatic meeting transitions: host (`+1`) plus current participant rows with status `JOINED`, `PENDING`, or `APPROVED`. The backend also applies automatic meeting transitions before mapping the schedule, so stale in-range meetings can be surfaced as `RECRUITMENT_CLOSED` or `COMPLETED` consistently with existing meeting reads.
...
- MeetingLog upload follow-up adds a separate upload helper endpoint. The backend still stores only photo URLs plus `sizeBytes` metadata in the log domain, but it now delegates actual file persistence and URL generation to S3 through the shared common storage layer, while the `meeting` domain keeps only the log-photo-specific validation and endpoint contract.
...
- Profile field note:
  ...
  - `escapeCount` is the number of completed meetings the searched user belongs to
  - a completed meeting counts when the user is the host, or when the user has participation history with status `JOINED`, `PENDING`, or `APPROVED`
  - `LEFT` participation does not count
