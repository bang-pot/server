...
public class RecordMeetingResultService implements RecordMeetingResultUseCase {
	...
	public Result handle(Command command) {
		...
		completedUserAccessService.validateCompletedUser(command.userId(), "가입한 크루원만 결과를 입력할 수 있습니다.");
		if (crewMemberRepository.findByCrewIdAndUserId(command.crewId(), command.userId()).isEmpty()) {
			throw new AccessDeniedException("가입한 크루원만 결과를 입력할 수 있습니다.");
		}
		...
		if (!meeting.getHostUserId().equals(command.userId())) {
			throw new AccessDeniedException("모임 개설자만 결과를 입력할 수 있습니다.");
		}

		meeting.recordResult(MeetingResult.valueOf(command.result()));
		meetingRepository.save(meeting);
		return Result.of(meeting.getId(), meeting.getResult().name());
	}
}
