...
interface MeetingGalleryJpaRepository extends JpaRepository<MeetingLog, Long> {
	...
	@Query("""
		select new com.banglog.meeting.domain.view.CrewMeetingGalleryDetailTargetView(
			m.id,
			m.meetingDate,
			m.title
		)
		...
		where m.crewId = :crewId
		  and m.id = :meetingId
		  and m.status = com.banglog.meeting.domain.MeetingStatus.COMPLETED
		  and exists (
				select 1
	...
	@Query("""
		select new com.banglog.meeting.domain.view.CrewMeetingGalleryDetailView$PhotoSource(
			mlp.id,
			mlp.photoUrl
		)
		from MeetingLog ml, MeetingLogPhoto mlp
