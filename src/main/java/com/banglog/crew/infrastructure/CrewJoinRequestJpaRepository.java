...
interface CrewJoinRequestJpaRepository extends JpaRepository<CrewJoinRequest, Long> {
	...
	@Query("""
		select new com.banglog.crew.domain.view.CrewJoinRequestManagementAccessView(
			member.role
		)
		from Crew c
	...
	@Query("""
		select new com.banglog.crew.domain.view.CrewJoinRequestsView$Item(
			request.id,
			user.id,
			user.nickname,
			request.message,
	...
	@Query("""
		select new com.banglog.crew.domain.view.PendingCrewJoinRequestsView$Item(
			request.id,
			user.id,
			user.nickname
		)
