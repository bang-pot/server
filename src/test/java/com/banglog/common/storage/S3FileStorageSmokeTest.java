...
class S3FileStorageSmokeTest {
	...
	void uploadsFileToS3AndExposesItThroughPublicUrl() throws Exception {
		...
		try {
			...
			assertThat(response.body()).isEqualTo("banglog-s3-smoke");
		} finally {
	...
	private FileStorageProperties loadProperties() {
		...
		properties.setPublicBaseUrl(required(source, "banglog.storage.public-base-url"));
		properties.getS3().setBucket(required(source, "banglog.storage.s3.bucket"));
		properties.getS3().setRegion(optional(source, "banglog.storage.s3.region", "ap-northeast-2"));
		properties.getS3().setAccessKey(required(source, "banglog.storage.s3.access-key"));
		properties.getS3().setSecretKey(required(source, "banglog.storage.s3.secret-key"));
		return properties;
	}
