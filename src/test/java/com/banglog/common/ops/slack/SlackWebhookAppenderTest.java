...
class SlackWebhookAppenderTest {
	...
	void targetsStartupInfoLog() {
		...
		assertThat(appender.isSlackTarget(loggingEvent(
			Level.INFO,
			"com.banglog.common.logging.StartupLifecycleLogger",
	...
	void skipsScannerStyleProtectedResourceWarning() {
		...
		assertThat(appender.isSlackTarget(loggingEvent(
			Level.WARN,
			"com.banglog.auth.infrastructure.logging.AuthAuditLogger",
	...
	void buildsPayloadWithTrackingFieldsAndReason() {
		...
		LoggingEvent event = loggingEvent(
			Level.ERROR,
			"com.banglog.common.logging.ServerErrorLoggingFilter",
		...
		assertThat(payload)
			...
			.contains("- service: banglog-backend\\n")
	...
	void buildsSlowRequestPayloadWithDuration() {
		...
		LoggingEvent event = loggingEvent(
			Level.WARN,
			"com.banglog.common.logging.RequestTracingFilter",
	...
	private SlackWebhookAppender appender() {
		...
		appender.setService("banglog-backend");
