...
import com.banglog.meeting.application.service.CreateMeetingLogService;
import com.banglog.meeting.application.service.DeleteMeetingLogService;
import com.banglog.meeting.application.service.GetMeetingLogDetailService;
import com.banglog.meeting.application.service.GetMyMeetingLogService;
...
abstract class AbstractMeetingLogServicesTest {
	...
	protected static final class InMemoryCrewRepository implements CrewRepository {
		...
		public Optional<Crew> findById(Long crewId) {
			return Optional.ofNullable(crews.get(crewId))
				.filter(crew -> crew.getStatus() == com.banglog.crew.domain.CrewStatus.ACTIVE);
		}
	...
	protected static final class InMemoryCrewMemberRepository implements CrewMemberRepository {
		...
		public boolean existsByCrewIdAndUserId(Long crewId, Long userId) {
			return members.values().stream().anyMatch(member ->
				member.getCrewId().equals(crewId)
					...
					&& member.getStatus() == com.banglog.crew.domain.CrewMemberStatus.ACTIVE
			);
		}
		...
		public boolean existsLeaderByCrewIdAndUserId(Long crewId, Long userId) {
			return members.values().stream().anyMatch(member ->
				member.getCrewId().equals(crewId)
					&& member.getUserId().equals(userId)
					&& member.getRole() == com.banglog.crew.domain.CrewRole.LEADER
					&& member.getStatus() == com.banglog.crew.domain.CrewMemberStatus.ACTIVE
			);
		}
	...
	protected static final class InMemoryMeetingRepository implements MeetingRepository {
		...
		public List<Meeting> findRecruitmentCloseTargets(java.time.LocalDateTime now, int limit) {
			return meetings.values().stream()
				.filter(meeting -> meeting.getStatus() == com.banglog.meeting.domain.MeetingStatus.RECRUITING)
		...
		public List<Meeting> findCompletionTargets(java.time.LocalDateTime completionCutoff, int limit) {
			return meetings.values().stream()
				.filter(meeting -> meeting.getStatus() == com.banglog.meeting.domain.MeetingStatus.RECRUITMENT_CLOSED)
	...
	protected static final class InMemoryMeetingQueryRepository implements MeetingQueryRepository {
		...
		@Override
		public Optional<com.banglog.meeting.domain.view.CrewMeetingGalleryDetailTargetView>
			findCrewMeetingGalleryDetailTargetView(Long crewId, Long meetingId) {
			return Optional.empty();
		}

		@Override
		public List<com.banglog.meeting.domain.view.CrewMeetingGalleryDetailView.Photo>
		...
		@Override
		public com.banglog.meeting.domain.view.MeetingsView findMeetingsViewByCrewId(
			Long crewId,
			int page,
			int size
		) {
			return com.banglog.meeting.domain.view.MeetingsView.of(
				List.of(),
				com.banglog.meeting.domain.view.MeetingsView.Page.of(page, size, false)
			);
		}

		@Override
		public Optional<com.banglog.meeting.domain.view.MeetingDetailView> findMeetingDetailView(
			Long crewId,
			Long meetingId,
			Long userId
		) {
	...
	protected static final class InMemoryMeetingLogQueryRepository implements MeetingLogQueryRepository {
		...
		@Override
		public com.banglog.meeting.domain.view.MyMeetingLogsView findMyMeetingLogsViewByAuthorUserId(
			Long userId,
			int page,
			int size
		) {
			return com.banglog.meeting.domain.view.MyMeetingLogsView.of(
				List.of(),
				com.banglog.meeting.domain.view.MyMeetingLogsView.Page.of(page, size, false)
			);
		}
